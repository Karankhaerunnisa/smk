...
<div x-data="{
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <button @click="openCreate()" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition text-sm font-medium flex items-center shadow-sm">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="p-4 text-sm text-gray-600">
                        <div class="flex flex-col text-xs">
                            <span>Mulai: {{ $item->published_at ? $item->published_at->format('d M Y H:i') : '-' }}</span>
                            <span class="text-gray-400">Selesai: {{ $item->expired_at ? $item->expired_at->format('d M Y H:i') : 'Selamanya' }}</span>
                        </div>
                    </td>
                    <td class="p-4 text-center">
                        ...
                        @else
                            <span class="px-2 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-600">Non-Aktif</span>
                        @endif
                    </td>
                    <td class="p-4 flex justify-center gap-2">
                        <button @click="openEdit({{ $item }})" class="p-2 bg-yellow-50 text-yellow-600 rounded-full hover:bg-yellow-100 transition">
                ...
                @empty
                <tr><td colspan="4" class="p-8 text-center text-gray-500">Belum ada pengumuman.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-2xl p-6">
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button @click="modalOpen = false"><x-heroicon-o-x-mark class="h-6 w-6 text-gray-400" /></button>
                </div>
                ...
                <form :action="formAction" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" x-model="data.title" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 text-sm">
                    </div>

                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Mulai Tayang</label>
                            <input type="datetime-local" name="published_at" x-model="data.published_at" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Selesai (Opsional)</label>
                            <input type="datetime-local" name="expired_at" x-model="data.expired_at" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 text-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="content" x-model="data.content" rows="6" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 text-sm"></textarea>
                    </div>

                    <div class="mb-6 flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" x-model="data.is_active" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        <label for="is_active" class="ml-2 block text-sm text-gray-900">Publikasikan (Aktif)</label>
                    </div>

                    <div class="flex justify-end gap-2">
                        <button type="button" @click="modalOpen = false" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md text-sm hover:bg-gray-200">Batal</button>
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-bold hover:bg-blue-700">Simpan</button>
                    </div>
                </form>
